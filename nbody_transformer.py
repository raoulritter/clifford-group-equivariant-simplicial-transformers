...
def make_edge_attr(node_features, edges, batch_size):
    ...
    for i in range(total_number_edges):
        ...
        # mean_vector = torch.stack((node_i_features, node_j_features))
        # mean =  torch.mean(mean_vector, dim=0)

        # geom product between node features
        # geom_product = clifford_algebra.geometric_product(node_i_features, node_j_features)

        # now stack all the 2 to get the "representation of the edge"
        # edge_representation = torch.cat((difference, geom_product), dim=0)
        # edge_representation = torch.cat((mean, geom_product), dim=0)
...
def embed_nbody_graphs(batch):
    ...
    return full_node_embedding, full_edge_embedding, loc_end_clifford
...
for epoch in tqdm(range(10)):
    for batch in train_loader:
        ...
        nodes, edges, tgt = components

        output = model(nodes, edges, src_mask=None, batch_size=batch_size)
