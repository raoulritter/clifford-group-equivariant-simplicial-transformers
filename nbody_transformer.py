...
class GAST_block(nn.Module):
    def __init__(self, d_model, num_heads, clifford_algebra, channels):
        ...
        self.cemlp = CEMLP(clifford_algebra, d_model, d_model, d_model)
    ...
    def forward(self, src, src_mask=None):
        ...
        src = self.mvlayernorm2(attn_output)
        src = self.cemlp(src)
        return src
...
class GAST(nn.Module):
    def __init__(self, num_layers, d_model, num_heads, clifford_algebra, channels):
        super(GAST, self).__init__()
        self.layers = nn.ModuleList([GAST_block(d_model, num_heads, clifford_algebra, channels) for _ in range(num_layers)])
        self.norm = nn.LayerNorm(d_model)
...
class NBODY_Transformer(nn.Module):
    def __init__(self, input_dim, d_model, num_heads, num_layers, mv_in_features, mv_out_features, clifford_algebra, channels, batch_size):
...
# Create the model
model = NBODY_Transformer(input_dim, d_model, num_heads, num_layers, mv_in_features, mv_out_features, clifford_algebra, channels, batch_size)
