...
def parse_arguments():
    ...
    parser.add_argument('--simplex_order', type=int, choices=[1, 2], default=1, help='Order of simplices')
    parser.add_argument('--weight_decay', type=float, default=0.00001, help='Weight decay')
    parser.add_argument('--early_stopping_limit', type=int, default=50, help='Early stopping limit')
    parser.add_argument('--empty_higher_simplices', action='store_true', help='Flag to indicate zero embedded simplices')
...
def save_losses_to_csv(args, train_losses, val_losses, test_loss, filename='losses.csv'):
    filename = f'../../results/{args.simplex_order}_{args.empty_higher_simplices}_{filename}'
...
def main():
    ...
    if args.test_only:
        model = NBodyTransformer(
            ...
            empty_higher_simplices=args.empty_higher_simplices,
            simplex_order=args.simplex_order
        )
        model.load_state_dict(torch.load(f'../../results/trained_models/{args.simplex_order}_{args.empty_higher_simplices}_best_model.pth'))
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'./{args.simplex_order}_{args.empty_higher_simplices}_best_model.pth')
            early_stopping_counter = 0
        else:
    ...
    model.load_state_dict(torch.load(f'./{args.simplex_order}_{args.empty_higher_simplices}_best_model.pth'))
    test_loss = test_model(model, test_loader, criterion)
    print(f'Test Loss: {test_loss}')
